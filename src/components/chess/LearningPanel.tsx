...
type LearningPanelProps = {
  ...
  onBackToMenu: () => void;
  // Les 3 variables de ton hook useTutorialManager :
...
export function LearningPanel({
  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-slate-600 bg-slate-800/70 p-4 shadow-lg shadow-slate-950/40">
      ...
      {/* Si l'ordinateur a plusieurs choix possibles, on affiche les boutons de sélection */}
      {isWaitingForBranchChoice ? (
        <div className="flex flex-col gap-3">
          <p className="rounded-lg border border-indigo-500/40 bg-indigo-500/20 p-3 text-sm font-semibold leading-relaxed text-indigo-100">
            Les blancs ont plusieurs solutions égales. Laquelle voulez-vous analyser ?
          </p>
          <div className="grid grid-cols-2 gap-2">
            {branchChoices.map((move) => (
              <button
                key={move}
                type="button"
                onClick={() => onPlayMove(move)}
                className="rounded-lg border border-indigo-400/40 bg-indigo-500/30 px-4 py-3 text-sm font-semibold text-indigo-50 transition hover:border-indigo-300/80 hover:bg-indigo-500/50"
              >
                Analyser {move}
              </button>
            ))}
        ...
        /* Sinon, affichage normal du texte et des options de fin de variante */
        <>
