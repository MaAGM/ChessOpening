...
type OpeningSelectorProps = {
  onSelectOpening: (id: string, path: string[]) => void;
};
    ...
    <section className="flex h-full flex-col gap-3">
      ...
      <ul className="space-y-3">
        {openingTutorials.map((opening) => {
          const rawProgress = learningProgress[opening.id];
          const progressPath = Array.isArray(rawProgress) ? rawProgress : [];
          const isResume = progressPath.length > 0;
            ...
            <li key={opening.id} className="rounded-xl border border-slate-700 bg-slate-900/40 p-3">
              ...
              {isResume && (
                <p className="mt-1 text-xs text-slate-400">
                  Progression: {progressPath.join(" ")}
                </p>
              )}

              <button
                type="button"
                onClick={() => onSelectOpening(opening.id, progressPath)}
